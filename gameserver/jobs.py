...
def start_background_jobs():
	...
	scheduler = AsyncIOScheduler(executors={'default': executor}, jobstores={'default':jobstore}, timezone=utc, event_loop=get_event_loop())
	scheduler.start()
	scheduler.print_jobs()
	if len(scheduler.get_jobs(jobstore)) == 0:
		try:
			scheduler.add_job(minute_job, trigger="cron", id='minute_job', minute='*/1')
			scheduler.add_job(five_minute_job, trigger="cron", id='five_minute_job', minute='*/5')
			...
			scheduler.add_job(daily, trigger="cron", id="daily", minute=0, hour=0, day="*/1")
			scheduler.add_job(weekly, trigger="cron", id="weekly", minute=0, hour=0, month="*/1", day_of_week='0')
		except Exception:
			pass
